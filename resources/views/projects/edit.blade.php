...
<div style="max-width: 800px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <h2 style="color: #2c3e50; font-size: 24px;">Edit Project: {{ $project->name }}</h2>
        ...
        <form action="{{ route('projects.update', $project) }}" method="POST">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="description" id="description" rows="5" required
                          style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px; resize: vertical;">{{ old('description', $project->description) }}</textarea>
                @error('description')
                    <span style="color: #e74c3c; font-size: 12px; margin-top: 5px; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select name="status" id="status" required
                    ...
                    <option value="deleted" {{ old('status', $project->status) === 'deleted' ? 'selected' : '' }}>Deleted</option>
                </select>
                @error('status')
                    <span style="color: #e74c3c; font-size: 12px; margin-top: 5px; display: block;">{{ $message }}</span>
                @enderror
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="text" name="responsible_person_first_name" id="responsible_person_first_name" value="{{ old('responsible_person_first_name', $project->responsible_person_first_name) }}" required
                           style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                ...
                <div>
                    ...
                    <input type="text" name="responsible_person_last_name" id="responsible_person_last_name" value="{{ old('responsible_person_last_name', $project->responsible_person_last_name) }}" required
                           style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $project->start_date->format('d-m-Y')) }}" required
                           style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                    @error('start_date')
                        <span style="color: #e74c3c; font-size: 12px; margin-top: 5px; display: block;">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $project->end_date->format('d-m-Y')) }}" required
                           style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                    @error('end_date')
                        <span style="color: #e74c3c; font-size: 12px; margin-top: 5px; display: block;">{{ $message }}</span>
                    @enderror
            ...
            <div style="display: flex; gap: 10px; margin-top: 30px;">
                <button type="submit" class="btn btn-warning" style="flex: 1;">Update Project</button>
